...
grid <- cbind.data.frame(coordinates(sp_points), sp_points$pm_2010)
names(grid) <- c("x", "y", "pm_2010")
...
#------------------ use code below to get PM2.5 maps for 2000, 2005, 2010, and 2015 in one plot
...
#png("results/figures/pm25_map.jpeg", height = 1024 * 0.53 * k, width = 1024 * k)
gc()
png("results/figures/pm25_maps.jpeg", height = 1024 * 0.65 * k, width = 1024 * k)
grid %>%
  ggplot() +
  geom_point(aes(x = x, y = y, color = pm25),
             size = 0.15 * k) +
